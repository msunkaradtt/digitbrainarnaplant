...
def __converdate(x):
    ...
    if len(timeStamp_) == 13:
        date_ = datetime.fromtimestamp(int(timeStamp_) / 1000).strftime("%d/%m/%Y")
    else:
        date_ = (datetime(1970, 1, 1) + timedelta(milliseconds = int(timeStamp_) % 1000)).strftime("%d/%m/%Y")

    return date_

@app.get("/")
async def root():
    endpoints = ['tasks', 'tools', 'machines']
    x = DataReader(endpoints)
    await x.fetch_main()
    
    taskList = ['UID', 'secondsPerProduct', 'toolCode', 'toolSize', 'dueDate', 'qtyTotal']
    toolList = ['id', 'toolSize', 'toolCode', 'total', 'available']
    machineList = ['id', 'secondsPerProduct', 'name', 'machines_id', 'machines_name']
    ...
    file_path_1 = data_dir_ + "/" + endpoints[0] + ".json"
    p1_data.to_json(file_path_1, indent=2, date_format='iso', date_unit='ms')
    file_path_2 = data_dir_ + "/" + endpoints[1] + ".json"
    p2_data.to_json(file_path_2, indent=2)
    file_path_3 = data_dir_ + "/" + endpoints[2] + ".json"
    p3_data.to_json(file_path_3, indent=2)
